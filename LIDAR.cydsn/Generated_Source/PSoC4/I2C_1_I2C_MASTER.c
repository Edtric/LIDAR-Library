...
uint32 I2C_1_I2CMasterSendStop(void)
{
    ...
    if(I2C_1_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!I2C_1_CHECK_INTR_MASTER(I2C_1_INTR_MASTER_I2C_STOP     |
                                                  I2C_1_INTR_MASTER_I2C_ARB_LOST |
                                                  I2C_1_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(I2C_1_CHECK_INTR_MASTER(I2C_1_INTR_MASTER_I2C_STOP))
        {
            errStatus = I2C_1_I2C_MSTR_NO_ERROR;
        }
        else if(I2C_1_CHECK_INTR_MASTER(I2C_1_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* I2C_1_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = I2C_1_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = I2C_1_I2C_RESET_ERROR;
        }
...
*
*******************************************************************************/
uint32 I2C_1_I2CMasterWriteByte(uint32 theByte)
{
    uint32 resetIp;
    uint32 errStatus;
    ...
    if(I2C_1_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        else /* I2C_1_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            I2C_1_state = I2C_1_I2C_FSM_IDLE;
                         errStatus = I2C_1_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = I2C_1_I2C_RESET_ERROR;
...
*
*******************************************************************************/
uint32 I2C_1_I2CMasterReadByte(uint32 ackNack)
{
    uint32 theByte;

    /* Return invalid byte in case BUS_ERR happen during receiving */
    theByte = I2C_1_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(I2C_1_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!I2C_1_CHECK_INTR_RX(I2C_1_INTR_RX_NOT_EMPTY)) &&
              (!I2C_1_CHECK_INTR_MASTER(I2C_1_INTR_MASTER_I2C_ARB_LOST |
                                                  I2C_1_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(I2C_1_CHECK_INTR_RX(I2C_1_INTR_RX_NOT_EMPTY))
        {
            theByte = I2C_1_RX_FIFO_RD_REG;
...
void I2C_1_I2CReStartGeneration(void)
{
    ...
    uint32 status = I2C_1_I2C_STATUS_REG;

    while(I2C_1_WAIT_SDA_SET_HIGH)
    {
    ...
    if(0u != (status & I2C_1_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        I2C_1_SET_I2C_SCL_DR(I2C_1_I2C_SCL_LOW);
        CyDelayUs(I2C_1_I2C_TLOW_TIME); /* Count tLOW */

        I2C_1_SET_I2C_SCL_DR(I2C_1_I2C_SCL_HIGH);
        while(I2C_1_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(I2C_1_I2C_THIGH_TIME); /* Count tHIGH */
    }
